...
class Controls:
  ...
  def controlsd_thread(self):
    ...
    contador = 0
    doc = open("/openpilot/selfdrive/controls/velocidad.txt",'a')
    doc.write("vEgo: "+"\t"+"cruiseState.speed"+"\t"+"actuators.speed"+ "\t"+"steeringAngleDeg: "+"\t"+"hudControl.setSpeed"+"\n")
    ...
    while True:
      ...
      if (contador%50) ==0:
        ...
        doc.write(str(KS.vEgo)[:5]+"\t"+ str(KS.cruiseState.speed)[:5]+"\t"+str(KC.actuators.speed)[:5]+"\t"+str(KC.hudControl.setSpeed)[:5]+"\n")
        #doc.write(str(KS.vEgo)[:5]+"\t"+ str(KS.steeringAngleDeg)[:5]+"\t"+str(KC.hudControl.setSpeed)[:5]+"\n")
      #time_start = dt.datetime.now()
      contador = contador + 1 
